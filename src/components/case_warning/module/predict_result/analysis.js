  ...
  render() {
    ...
    return (<div>
      <div className="analysis_title">
        <i></i>  {!analysisType ? '刑期分布统计图' : analysisType === 1 ? '罚金统计分布图' : '缓刑统计分布图'}
      </div>
      <ul className='analysis_type'>
        <li
      ...
      </ul>
      {
        (!analysisType && this.mainShow) || (analysisType === 1 && this.fineShow) || (analysisType === 2 && this.probationShow) ?
          <div className='analysis_small'>
            {mapData[0][0] || mapData[1][0] || mapData[2][0] ?
              <ReactEcharts
